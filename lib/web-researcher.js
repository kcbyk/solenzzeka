...
async function performResearch(query, depth = 1) {
  ...
  if (wikiResults.length > 0) {
    const knowledge = wikiResults.map(r => r.text).join('\n\n');
    nlp.addDocument(`Odaklı: ${query}`, knowledge);
    
    // DERİNLEŞME: Eğer derinlik > 0 ise, bu konudan 2 alt başlık çıkarıp onları da araştır
    if (depth > 0) {
      const subTopics = extractSubTopics(wikiResults[0].text, query);
      for (const sub of subTopics) {
        // Alt başlıkları sessizce araştır ve öğren
        await performResearch(sub, depth - 1);
      }
    }

    return { success: true, summary: wikiResults[0].text };
  }
