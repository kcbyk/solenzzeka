...
function processText(text) {
  // Gereksiz karakterleri ve çok kısa kelimeleri temizle
  const cleanText = text.replace(/\[\d+\]/g, '') // [1], [2] gibi kaynakları temizle
                        .replace(/[^\w\sçşğıöüâîûÇŞĞİÖÜ.?!]/g, ' ')
                        .replace(/\s+/g, ' ');
  
  return tokenize(cleanText).map(t => stem(t));
}
...
function findAnswer(query, topK = 3) {
  if (knowledgeBase.chunks.length === 0) {
    return {
      answer: 'Şu an hafızam boş görünüyor. Hemen senin için internette araştırma başlatıyorum, lütfen bir saniye bekle... 🔍',
      confidence: 0,
      sources: [],
      noData: true
    };
